...
from atop_handler import AtopHandler
...
atop_stats = multiprocessing.Queue(20)
store = None
...
def collect_atop_stats(root, parents, data, meta, coll,
                     key, val, meta_val, meta_inf, level=0):

    if not isinstance(val, list):
        logging.error(
            "unable to collect stats: val must be a list - %s" % val)
        return False


def atop_worker(jobs, stats, ctl, store, timeout=5):
    logging.info("atop_worker started")

    while ctl["run_ok"]:
        try:
            root, parents, val = jobs.get(block=True, timeout=timeout)
        except Queue.Empty:
            logging.debug("atop_worker hasn't received jobs for %s seconds"
                          % timeout)
            continue
