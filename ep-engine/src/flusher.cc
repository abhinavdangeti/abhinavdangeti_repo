...
void Flusher::schedule_UNLOCKED() {
    ...
    taskId = iom->scheduleFlusherTask(ObjectRegistry::getCurrentEngine(),
...
bool Flusher::step(size_t tid) {
    //std::cout << "flusher state: " << stateName(_state) << std::endl;
    try {
        switch (_state) {
        case initializing:
            //std::cout << "initializing loop: " <<  stateName(_state) << std::endl;
            initialize(tid);
            return true;
        case paused:
            return false;
        ...
        case running:
            {
                //std::cout << "running loop: " << stateName(_state) << std::endl;
                doFlush();
                if (_state == running) {
