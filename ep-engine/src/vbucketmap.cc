...
std::vector<int> VBucketMap::getBuckets(uint16_t shardId) {
    std::vector<int> rv;
    size_t maxVbuckets = size;
    if (shardId >= numShards) {
        LOG(EXTENSION_LOG_WARNING,
            "No vbucket can be found for invalid shard = %d", shardId);
        return rv;
    }
    KVShard *shard = shards[shardId];
    for (size_t i = 0; i < maxVbuckets; ++i) {
        RCPtr<VBucket> b(shard->getBucket(i));
        if (b) {
            rv.push_back(b->getId());
        }
    }
