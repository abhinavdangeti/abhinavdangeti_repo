...
ENGINE_ERROR_CODE EventuallyPersistentEngine::getStats(const void* cookie,
    ...
    if (stat_key == NULL) {
        ...
        getEpStore()->getOneROUnderlying()->addTimingStats("ro", add_stat, cookie);
        getEpStore()->getOneRWUnderlying()->addTimingStats("rw", add_stat, cookie);
        rv = ENGINE_SUCCESS;
    } else if (nkey == 7 && strncmp(stat_key, "kvstore", 7) == 0) {
        // FIXME - We need to aggregate all of the shards
        getEpStore()->getOneROUnderlying()->addStats("ro", add_stat, cookie);
        getEpStore()->getOneRWUnderlying()->addStats("rw", add_stat, cookie);
        rv = ENGINE_SUCCESS;
            ...
            if ((bodylen - keylen) == 0) {
                ...
                if (!es) {
                    uint16_t persisted_chk_id =
                        epstore->getLastPersistedCheckpointId(vbucket);
