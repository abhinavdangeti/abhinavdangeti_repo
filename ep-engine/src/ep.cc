...
EventuallyPersistentStore::EventuallyPersistentStore(EventuallyPersistentEngine &theEngine) :
    ...
    statsSnapshotTaskId(0), mLogCompactorTaskId(0)
{
...
const Flusher* EventuallyPersistentStore::getFlusher() {
    // FIXME - this method is used for collecting flusher stat
    //         per bucket. With multiple flushers running on
    //         a single bucket. Until we provide per shard
    //         stat just pick one flusher and provide its stat
    return vbMap.getShard(0)->getFlusher();
}
...
bool EventuallyPersistentStore::startFlusher() {
    // FIXME - invocate each shard's deamon tasks from global io task
    //         manager or here
    for (uint16_t i = 0; i < vbMap.numShards; ++i) {
        Flusher *flusher = vbMap.shards[i]->getFlusher();
        if (flusher == NULL) {
            for (uint16_t j = 0; j < i; ++j) {
                flusher = vbMap.shards[j]->getFlusher();
                flusher->stop(true);
            }
            LOG(EXTENSION_LOG_WARNING,
                "Failed to start flusher for shard %d", i);
            return false;
        }
        flusher->start();
    }
...
bool EventuallyPersistentStore::pauseFlusher() {
    ...
    for (uint16_t i = 0; i < vbMap.numShards; i++) {
        ...
        if (!flusher->pause()) {
            LOG(EXTENSION_LOG_WARNING,
                "Warning: attempted to pause flusher in state [%s], shard = %d",
                flusher->stateName(), i);
            rv = false;
        }
    }
...
void EventuallyPersistentStore::snapshotVBuckets(const Priority &priority,
    ...
    public:
        VBucketStateVisitor(EventuallyPersistentStore &s) : store(s) { }
        bool visitBucket(RCPtr<VBucket> &vb) {
            vbucket_state vb_state;
            vb_state.state = vb->getState();
            const VBucketMap &map = store.getVBuckets();
            vb_state.checkpointId = map.getPersistenceCheckpointId(vb->getId());
...
void EventuallyPersistentStore::snapshotStats() {
    ...
    // TODO we may want to split stats json file into per shard stats
    //      json. Til then just pick one shard's underlying to do
    //      snapshotStats for the entire database because CouchKVStore
    //      doesn't support write synchronization
...
int EventuallyPersistentStore::flushVBucket(uint16_t vbid) {
    ...
    if (vb && !vbMap.isBucketCreation(vbid)) {
        ...
        if (!items.empty()) {
            //std::cout << "Trying to flush " << items.size() << " items" << std::endl;
            ...
            //std::cout << "Flushed " << items_flushed << " items" << std::endl;
        }
    }
...
{
    // randomly pick one roUnderlying for a global task like
    // warmup, etc.
